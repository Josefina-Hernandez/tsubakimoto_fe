        ...
        <div class="table-container">
          <table>
            ...
            <tbody>
              ...
              >
                <td>
                  <input class="checker" type="checkbox" v-model="item.checked" @change="handleSelect(item)" @click.stop>
                </td>
                <td>{{ item.category }}</td>
                <td>{{ (item.partNo != null && item.partNo != '-' && item.partNo != '') ? (Number(item.partNo) > 9999 ? String(item.partNo).padStart(5, '0') : String(item.partNo).padStart(4, '0'))  : '' }}</td>
  ...
  methods: {
    ...
    addProductNormal() {
      if (this.MODE == 0) {
        ...
        this.selectedDataLine.type = 'Online Price List';
